...
{
    use \Wikichua\Dashing\Http\Traits\ModelScopes;
    use \Wikichua\Dashing\Http\Traits\Searchable;
    use \Wikichua\Dashing\Http\Traits\DynamicFillable;
    use \Wikichua\Dashing\Http\Traits\UserTimezone;
    ...
    protected function snapshotIt($mode = 'Updated')
    {
        if ('created' != strtolower($mode) && !\Str::contains(get_class($this), ['Searchable', 'Audit', 'Alert', 'Versionizer']) && isset($this->snapshot) && $this->snapshot) {
            ...
            if (count($changes) || 'deleted' == strtolower($mode)) {
                ...
                app(config('dashing.Models.Versionizer'))->create([
                    'mode' => $mode,
