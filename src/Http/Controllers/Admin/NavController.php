...
class NavController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $models = app(config('dashing.Models.Nav'))->query()
                ->with('brand')
                ->checkBrand()
    ...
    public function store(Request $request)
    {
        ...
        $model = app(config('dashing.Models.Nav'))->create($request->all());
        sendAlert([
    ...
    public function show($id)
    {
        ...
        $model = app(config('dashing.Models.Nav'))->query()->findOrFail($id);
    ...
    public function pages($brand_id)
    {
        $pages = [];
        if ('' != $brand_id) {
            $slugs = app(config('dashing.Models.Page'))->query()
    ...
    public function orderableUpdate(Request $request, $orderable = '', $brand_id = '')
    {
        ...
        $models = app(config('dashing.Models.Nav'))->query()->select('id')
    ...
    public function migration(Request $request, $id)
    {
        ...
        $string = <<<EOL
            \$brand = app(config('dashing.Models.Brand'))->query()->where('name','{$brandString}')->first();
            app(config('dashing.Models.Nav'))->query()->create({$code});
            EOL;
