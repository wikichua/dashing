...
class PageController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $models = app(config('dashing.Models.Page'))->query()
                ->with('brand')
                ->checkBrand()
    ...
    public function store(Request $request)
    {
        ...
        $model = app(config('dashing.Models.Page'))->create($request->all());
    ...
    public function show($id)
    {
        ...
        $model = app(config('dashing.Models.Page'))->query()->findOrFail($id);
    ...
    public function templates($brand_id)
    {
        $templates = [];
        if ('' != $brand_id) {
            $model = app(config('dashing.Models.Brand'))->query()->findOrFail($brand_id);
            if ($model) {
    ...
    public function migration(Request $request, $id)
    {
        ...
        $string = <<<EOL
            ...
            app(config('dashing.Models.Page'))->query()->create({$code});
            EOL;
