...
namespace Wikichua\Dashing\Repos;
...
class Help
{
    ...
    public function getModels()
    {
        ...
        foreach ($autoload as $namespace) {
            if (Str::contains($namespace, ['Wikichua','Brand','App']) && !in_array($namespace, ['Wikichua\Dashing\Models\User','Wikichua\Dashing\Models\Searchable'])) {
    ...
    public function getBrand($brandName)
    {
        $brand = cache()->tags('brand')->remember('register-'.$brandName, (60 * 60 * 24), function () use ($brandName) {
            return app(config('dashing.Models.Brand'))->query()
    ...
    {
        $model = app(config('dashing.Models.Page'))->query()
    ...
    public function subPathRoutes($brandName, $controller)
    {
        $models = cache()->tags('page')->remember('page-'.$brandName, (60 * 60 * 24), function () use ($brandName) {
            ...
            if ($brand) {
                return app(config('dashing.Models.Page'))->query()
                ...
                dispatch(function () use ($data) {
                    ...
                    foreach ($receiver_ids as $receiver_id) {
                        ...
                        if ($data['receiver_id'] != auth()->id()) {
                            app(config('dashing.Models.Alert'))->create($data);
                        }
                    }
                });
            } else {
    ...
    {
        $permission = app(config('dashing.Models.Permission'))->where('name', str_slug($permission))->first();

        return cache()->tags(['permissions'])->rememberForever('permission_users:'.$permission->id.':'.$brand_id, function () use ($permission, $brand_id) {
            $ids = [];
            $ids = app(config('dashing.Models.Role'))->where('name', 'Admin')->first()->users()->whereNull('brand_id')->pluck('users.id')->toArray();
