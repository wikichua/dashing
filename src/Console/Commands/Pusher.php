...
{
    protected $signature = 'dashing:run:pusher {--brand=}';
    ...
    public function handle()
    {
        ...
        $pushers = app(config('dashing.Models.Pusher'))->query()->where('status', 'A')->whereBetween('scheduled_at', [$now, $now->addMinute()]);
        $brand = $this->option('brand');
        if ('' != $brand) {
            $brand = app(config('dashing.Models.Brand'))->query()->where('name', $brand)->where('status', 'A')->first();
            if ($brand) {
