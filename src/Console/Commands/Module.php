...
{
    protected $signature = 'dashing:make:module {model} {--brand=} {--force}';
    ...
    public function __construct(Filesystem $file)
    {
        parent::__construct();
        $this->file = $file;
        $this->stub_path = config('dashing.stubs.path');
    }
        ...
        foreach ($config_form as $field => $options) {
            ...
            if (isset($options['model_options']) && '' != $options['model_options']) {
                ...
                if (isset($options['options']) && is_array($options['options']) && count($options['options'])) {
                    ...
                    $settings_options_up[] = "app(config('dashing.Models.Setting'))->create(['created_by' => \$user_id, 'updated_by' => \$user_id, 'key' => '{$setting_key}','value' => [".implode(',', $opts).']]);';
                    $settings_options_down[] = "app(config('dashing.Models.Setting'))->where('key','{$setting_key}')->forceDelete();";
            ...
            switch ($options['type']) {
                ...
                case 'text':
                case 'url':
                    $form_stub = '<x-dashing::input-field type="'.$options['type'].'" name="{%field%}" id="{%field%}" label="{%label%}" class="{%class_tag%}" {%attributes_tag%} :value="$model->{%field%} ?? \'\'"/>';
                    $read_stub = '<x-dashing::display-field name="{%field%}" id="{%field%}" label="{%label%}" :value="$model->{%field%}" type="text"/>';

                    break;

                case 'time':
                    $form_stub = '<x-dashing::time-field name="{%field%}" id="{%field%}" label="{%label%}" class="{%class_tag%}" {%attributes_tag%} :value="$model->{%field%} ?? \'\'"/>';
                    $read_stub = '<x-dashing::display-field name="{%field%}" id="{%field%}" label="{%label%}" :value="$model->{%field%}" type="text"/>';

                    break;

                case 'date':
                    $form_stub = '<x-dashing::date-field name="{%field%}" id="{%field%}" label="{%label%}" class="{%class_tag%}" {%attributes_tag%} :value="$model->{%field%} ?? \'\'"/>';
                    $read_stub = '<x-dashing::display-field name="{%field%}" id="{%field%}" label="{%label%}" :value="$model->{%field%}" type="date"/>';

                    break;

                case 'datetime':
                    $form_stub = '<x-dashing::datetime-field name="{%field%}" id="{%field%}" label="{%label%}" class="{%class_tag%}" {%attributes_tag%} :value="$model->{%field%} ?? \'\'"/>';
                    $read_stub = '<x-dashing::display-field name="{%field%}" id="{%field%}" label="{%label%}" :value="$model->{%field%}" type="datetime"/>';

                    break;

                case 'image':
                    $form_stub = '<x-dashing::image-field type="'.$options['type'].'" name="{%field%}" id="{%field%}" label="{%label%}" class="{%class_tag%}" {%attributes_tag%} :value="$model->{%field%} ?? \'\'"/>';
                    $read_stub = '<x-dashing::display-field name="{%field%}" id="{%field%}" label="{%label%}" :value="$model->{%field%}" type="image"/>';

                    break;

                case 'file':
                    $form_stub = '<x-dashing::file-field type="'.$options['type'].'" name="{%field%}" id="{%field%}" label="{%label%}" class="{%class_tag%}" {%attributes_tag%} :value="$model->{%field%} ?? \'\'"/>';
                    $read_stub = '<x-dashing::display-field name="{%field%}" id="{%field%}" label="{%label%}" :value="$model->{%field%}" type="file"/>';

                    break;

                case 'textarea':
                    $form_stub = '<x-dashing::textarea-field name="{%field%}" id="{%field%}" label="{%label%}" class="{%class_tag%}" {%attributes_tag%} :value="$model->{%field%} ?? \'\'"/>';
                    $read_stub = '<x-dashing::display-field name="{%field%}" id="{%field%}" label="{%label%}" :value="$model->{%field%}" type="text"/>';
                ...
                case 'radio':
                    $form_stub = '<x-dashing::radios-field name="{%field%}" id="{%field%}" label="{%label%}" :options="'.$select_options.'" :checked="$model->{%field%} ?? []" :isGroup="false" :stacked="'.($options['stacked'] ? 1 : 0).'"/>';
                    $read_stub = '<x-dashing::display-field name="{%field%}" id="{%field%}" label="{%label%}" :value="$model->{%field%}" type="text"/>';

                    break;

                case 'checkbox':
                    $form_stub = '<x-dashing::checkboxes-field name="{%field%}" id="{%field%}" label="{%label%}" :options="'.$select_options.'" :checked="$model->{%field%} ?? []" :isGroup="false" class="{%class_tag%}" :stacked="'.($options['stacked'] ? 1 : 0).'"/>';
                    $read_stub = '<x-dashing::display-field name="{%field%}" id="{%field%}" label="{%label%}" :value="$model->{%field%}" type="list"/>';

                    break;

                case 'editor':
                    $form_stub = '<x-dashing::editor-field name="{%field%}" id="{%field%}" label="{%label%}" class="{%class_tag%}" {%attributes_tag%} :value="$model->{%field%} ?? \'\'"/>';
                    $read_stub = '<x-dashing::display-field name="{%field%}" id="{%field%}" label="{%label%}" value="{!! $model->{%field%} !!}" type="editor"/>';

                    break;

                case 'markdown':
                    $form_stub = '<x-dashing::markdown-field name="{%field%}" id="{%field%}" label="{%label%}" class="{%class_tag%}" {%attributes_tag%} :value="$model->{%field%} ?? \'\'"/>';
                    $read_stub = '<x-dashing::display-field name="{%field%}" id="{%field%}" label="{%label%}" value="{!! $model->{%field%} !!}" type="markdown"/>';
                ...
                if (isset($options['attributes']['multiple']) && 'multiple' == $options['attributes']['multiple']) {
                    $upload_strings[] = <<<EOT
                                ...
                                if (\$request->hasFile('{$field}')) {
                                    ...
                                    {
                                        // \$uploaded_files[] = \$request->file('{$field}.'.\$key)->store('public/{$model_variable}/{$field}');
                    ...
                    $upload_strings[] = <<<EOT
                                if (\$request->hasFile('{$field}')) {
                                    // \$path = \$request->file('{$field}')->store('public/{$model_variable}/{$field}');
    ...
    protected function menu()
    {
        ...
        if ($this->brand) {
            ...
            $toWriteInFile = resource_path('views/vendor/dashing/components/custom-admin-menu.blade.php');
        }
        ...
        if (false === strpos($toWriteInFileContent, $replaceContent)) {
            ...
            $this->line('Menu included: <info>'.config('dashing.routes_dir').'</info>');
        }
    }
    ...
    protected function migration()
    {
        ...
        $filename = "dashing{$this->model}Table.php";
        if ($this->brand) {
