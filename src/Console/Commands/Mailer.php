...
{
    protected $signature = 'dashing:make:mailer {name} {--brand=} {--force}';
    ...
    protected function app()
    {
        ...
        $filename = "dashing{$this->name}MailSeed.php";
    ...
    protected function mailString($isBrand = 0)
    {
        ...
        return <<<EOT
            ...
            use Wikichua\\Dashing\\Http\\Traits\\MailableTrait;
            ...
            class {$name} extends TemplateMailable
            {
                ...
                /*
                Usage :
                \$user = app(config('dashing.Models.User'))->find(2);
    ...
    protected function seedString($isBrand = 0)
    {
        ...
        return <<<EOT
            ...
            class dashing{$name}MailSeed extends Migration
            {
                public function up()
                {
                    app(config('dashing.Models.Mailer'))->create([
                ...
                {
                    app(config('dashing.Models.Mailer'))->where('mailable', {$namespace}\\{$name}::class)->forceDelete();
                }
            }
